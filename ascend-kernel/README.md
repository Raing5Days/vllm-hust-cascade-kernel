...
| **D3 剖因**（`torch_npu.profiler` 硬件时长 + 逐 pipe 占用，主 shape M=2048/K=5120 bf16） | mode0 54.45µs：**vec 0.629 / scalar 0.367** / mte2 0.217 / mte3 0.167；mode1 50.66µs（vec 0.789）；mode2 56.39µs（vec 0.635）。**同 62.91MB 的 CANN `aclnnAdd` 只需 17.54µs** ⇒ **3.10x 结构性头寸**，且 mte2 仅占 0.22（搬得动） | 归因：每行 17 条向量指令里 **10 条只服务 1 个标量**（Rsqrt+Newton 链 9 条 count-1 + 1 个 V→S barrier），且行间**无预取**（深度 2 队列未跨行流水） |
...
| f3_floor 设计（F3 地板阶梯：配对 gate/up GEMM + 8 mode epilogue（0..5、7、8）；**按原设计判 NO-GO ⇒ 不立项**） | `csrc/ops/f3_floor/design.md`；判决与实测见 `profiles/qwen14b-instruct-hotspot-20260910/probe-f3/PHASE-B.md` |
