...
// ============================================================================
// fia_grain_floor - MEASUREMENT-ONLY copy of fa_fp32_stage1 (see design.md).
//
// Purpose: scan the KV搬运粒度 (blockStackNum, the number of paged-KV blocks
// stacked into one K/V transfer chunk) as a single variable, to test whether
// coarser KV搬运 lowers the AIC MTE2 busy time. It is a byte copy of the
// production op with one knob parameterised; it is NOT wired into any e2e path,
// is NOT part of any plugin bundle, and must never be used in production.
//
...
// CATLASS_ARCH is defined source-locally BEFORE any catlass include (the catlass
// 910B block/epilogue templates are gated on it, same as the upstream examples
// build). Deliberately NOT a target-wide -D: that leaked into the lse_merge
// compile of the same kernel target and broke its fp32 path on device (keep
// build knobs source-local; see plan-20260903 execution log).
#ifndef CATLASS_ARCH
...
// Copied from fa_fp32_stage1 (which itself ports catlass example 23): the
// FAInferKernel class body is byte-identical to the production op; the only
// entry point here is the fp32-out `fia_grain_floor` (the bf16 `FAInferBf16`
// entry is dropped - it would collide with the same symbol in the production
// fa target at link time).
...
class FAInferKernel {
  public:
    ...
    CATLASS_DEVICE void operator()<AscendC::AIV>(FAIKernelParams const &params) {
        ...
        if constexpr (std::is_same_v<ElementO, float>) {
        ...
        for (uint32_t taskIdx = coreIdx; taskIdx < totalTaskNum; taskIdx += uint32_t(coreNum)) {
            ...
            for (uint32_t kvSIdx = 0; kvSIdx < kvSLoopNumNoMask; kvSIdx += blockStackNum) {
                ...
                if (kvSIdx >= preLaunch * blockStackNum) {
                    ...
                    if constexpr (std::is_same_v<ElementO, float>) {
                        epilogueRescaleO(
                            gO[gmOffsetO], gOTmp[gmOffsetOTmp], layoutO, layoutOTmp, actualBlockShapePV,
                            qSBlockSize, qNBlockSize, (stackSeqCount - preLaunch == 0), 0, curStackTileMod,
                            gLse, qHeads, oBatchOffset / (qHeads * embed) + qSBlockIdx * curQSBlockTile, qStartNIdx);
                    } else {
                        epilogueRescaleO(
            ...
            for (uint32_t kvSIdx = maskedStartIdx; kvSIdx < kvSLoopNumTotal + preLaunchStackNum;) {
                ...
                if (kvSIdx >= preLaunchStackNum) {
                    ...
                    if constexpr (std::is_same_v<ElementO, float>) {
                        epilogueRescaleO(
                            gO[gmOffsetO], gOTmp[gmOffsetOTmp], layoutO, layoutOTmp, actualBlockShapePV,
                            qSBlockSize, qNBlockSize, (stackSeqCount - preLaunch == 0),
...
// fp32-out kernel entry (the production fa_fp32_stage1 entry, renamed): same
// dataflow as the bf16 variant, but the final O is written as fp32
// (skips the fp32->bf16 RescaleO output cast) and the fp32 LSE is emitted to `lse`
// with layout [numTokens, numHeads].
CATLASS_GLOBAL void fia_grain_floor(
    GM_ADDR q,
    GM_ADDR k,
    GM_ADDR v,
    GM_ADDR mask,
    ...
    // ffts (cross-core sync base) is injected by the ascendc framework launcher
    // (GetAscendCoreSyncAddr -> set_ffts_base_addr in the auto-gen wrapper); the
    // example's manual rtGetC2cCtrlAddr + SetSyncBaseAddr path is unused here.
    ...
    FAIKernelParams params{q, k, v, mask, blockTables, actualQseqlen, actualKvseqlen, o, s, p, oTemp, oUpdate,
                           tiling, lse};
