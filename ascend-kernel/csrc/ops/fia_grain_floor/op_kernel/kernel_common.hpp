...
// Ported verbatim from catlass example 23 (kernel_common.hpp, probe version) for
// fa_fp32_stage1 (plan-20260903 M-B step 2). FATilingData moved to the shared
// fai_tiling_data.hpp (host + device single source of truth).

#ifndef KERNEL_COMMON
#define KERNEL_COMMON

#include "fai_tiling_data.hpp"
...
struct FAIKernelParams {
    ...
    GM_ADDR oUpdate;
    GM_ADDR tiling;
    GM_ADDR lse; // fp32 [numTokens, numHeads], fp32-out kernel only; nullptr otherwise
    // Methods
    CATLASS_DEVICE
    FAIKernelParams() {
    }
