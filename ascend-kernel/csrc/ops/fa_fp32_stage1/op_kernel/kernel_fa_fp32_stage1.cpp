...
// Ported for fa_fp32_stage1 (plan-20260903 M-B step 2): verbatim from catlass example 23
// (probe version) except: FAInferFp16 entry removed, FAInferBf16Fp32Out renamed
// fa_fp32_stage1. FAInferKernel class body is byte-identical to the example.
...
// fp32-out kernel entry (fa_fp32_stage1, renamed from probe FAInferBf16Fp32Out):
// same dataflow as FAInferBf16, but the final O is written as fp32
// (skips the fp32->bf16 RescaleO output cast) and the fp32 LSE is emitted to `lse`
// with layout [numTokens, numHeads].
CATLASS_GLOBAL void fa_fp32_stage1(
    GM_ADDR q,
    GM_ADDR k,
    GM_ADDR v,
    GM_ADDR mask,
